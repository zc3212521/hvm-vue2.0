  ...
  <div>
    ...
    >
      <template slot="detail" slot-scope="text">
        <overflow-tool-tip :text="text" :len="80"/>
      </template>
    </a-table>
  </div>
</template>
...
import { getSysGlobalInstanceDruid } from '@/http/system'
import OverflowToolTip from '@/components/business/OverflowToolTip'

export default {
  name: 'druid',
  components: { OverflowToolTip, HTopBar, CustomFormItem },
  data () {
    return {
      flod: true,
      searchFormItem: [],
      ...
      columns: [
        ...
        { title: 'value', dataIndex: 'value', width: '50', scopedSlots: { customRender: 'detail' } },
        { title: 'datasource', dataIndex: 'datasource' }
      ],
      data: [],
      initData: [],
      tempData: []
  ...
  methods: {
    ...
    getDruid () {
      ...
      getSysGlobalInstanceDruid(params, null).then(
        res => {
          if (res.data.code === 99998 || res.data.code === 99999) {
            this.$message.error(res.data.msg)
          } else {
            for (let idx in res.data) {
              let datasource = res.data[idx]
              for (let key in datasource) {
                let value = datasource[key]
                if (value && Object.prototype.toString.call(value) === '[object Array]' && value.length > 0) {
                  value = '[ ' + value.join(',') + ' ]'
                }
                value = '' + value
                this.initData.push({
