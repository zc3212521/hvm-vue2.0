...
const jsePublicKey = env.jsePublicKey
const jsePrivateKey = ''
...
const addLeafInTree = (data, parentLeaf = -1) => {
  if (data.some(item => item.actionType === 1)) {
    data.sort((a, b) => a.actionType - b.actionType)
  }
  data.forEach((item, n) => {
    if (parentLeaf === -1) {
      item.leaf = n + ''
...
const translateTree = (data) => {
  data.forEach(item => {
    if (item.children.some((it) => it.children && it.children.length > 0)) {
      item.auth = [{
        authName: '访问',
        action: 'display',
        permission: true,
...
const formatAuth = (data, authList = []) => {
  data.forEach((item, index) => {
    ...
    if (item.children.length) {
      ...
      if (item.actionType) authList.sort((a, b) => a.meta.actionType - b.meta.actionType)
    }
  })
}
...
/**
 * 如果是顶级菜单，返回此顶级菜单下第一个有权限可点击的菜单；如果不是顶级菜单，返回null；如果顶级菜单下没有子菜单，返回null
  ...
  if (redirectedFromPath && menuTree) {
    for (let i = 0; i < menuTree.length; i++) {
      if (redirectedFromPath === menuTree[i].path) {
        let firstChildMenu = getFirstLinkMenu(menuTree[i])
        // 如果获取的第一个可以点击的菜单就是当前菜单，返回null
        return firstChildMenu === menuTree[i] ? null : firstChildMenu.name
      }
    }
  }
  return null
...
/**
 * 获取当前menu对象下第一个可点击的菜单；如果有子菜单返回最下层子菜单中第一个；如果没有返回当前菜单
 * @param menuItem
 * @returns {*}
 */
const getFirstLinkMenu = (menuItem) => {
  if (menuItem.children && menuItem.children.length > 0) {
    return getFirstLinkMenu(menuItem.children[0])
  }
  return menuItem
}
